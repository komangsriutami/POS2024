...
class DefectaCronTL extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'defectatl:cron';
    ...
    public function handle()
    {
        ...
        $cek = DB::connection($this->getConnectionName())->table('tb_bantu_transaksi_update_tl')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
            $last_id_obat_ex = $cek->last_id_obat_after;
            if($last_id_obat_ex >= $last_id_obat) {
                # selesai : 1. hapus data di tb_bantu_transaksi_update_tl dan ulang dari 0
                DB::connection($this->getConnectionName())->table('tb_bantu_transaksi_update_tl')->truncate();
                $skip = 1;
            } else {
        ...
        if($skip != 1) {
            DB::connection($this->getConnectionName())->table('tb_bantu_transaksi_update_tl')
