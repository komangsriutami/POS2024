...
class HistoriCronCK extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historick:cron';
    ...
    public function handle()
    {
        ...
        $cek = DB::table('tb_bantu_update_ck')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
            $last_id_obat_ex = $cek->last_id_obat_after;
            if($last_id_obat_ex >= $last_id_obat) {
                # selesai : 1. hapus data di tb_bantu_update_ck dan ulang dari 0
                DB::table('tb_bantu_update_ck')->truncate();
                $skip = 1;
            } else {
        ...
        if($skip != 1) {
            ...
            if($x > 0) { 
                \Log::info("Cron CK history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron CK history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron CK history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron CK history is working fine!");
    }
}
