        ...
        if ($this->is_penjualan_tanpa_item == 0) {
            foreach ($detail_penjualans as $detail_penjualan) {
                if(!in_array($detail_penjualan['id_obat'], $array_id_obat)){
                    ...
                    $stok_harga = DB::connection($this->getConnectionDefault())->table('tb_m_stok_harga_'.$inisial)->where('id_obat', $detail_penjualan['id_obat'])->first();
                    $obj->id_nota = $this->id;
                    ...
                    if($kurangStok['status'] == 0) {
                        ...
                        if($is_history == 1) {
                            ...
                            # create histori
                           /* $histori_stok = HistoriStok::on($this->getConnectionDefault())->where('id_obat', $obj->id_obat)->where('jumlah', $obj->jumlah)->where('id_jenis_transaksi', 1)->where('id_transaksi', $obj->id)->first();
