...
class TransaksiTO extends Model
{
    ...
    public function save_from_array($detail_transfer_outlets, $val){
        ...
        $apotek2 = MasterApotek::on($this->getConnectionName())->find($this->id_apotek_tujuan);
        ...
        foreach ($detail_transfer_outlets as $detail_transfer_outlet) {
            if(!in_array($detail_transfer_outlet['id_obat'], $array_id_obat)){
                ...
                if(isset($this->is_from_transfer) AND $this->is_from_transfer == 1) {
                    $tf = TransaksiTransferDetail::on($this->getConnectionName())->find($detail_transfer_outlet['id_detail_transfer']);
                    $tf->is_status = 1;
                    ...
                    if($tf->id_defecta != null OR $tf->id_defecta != 0) {
                        DefectaOutlet::on($this->getConnectionName())->where('id', $tf->id_defecta)->update(['jumlah' => $obj->jumlah, 'id_process' => 2, 'last_update_process' => date('Y-m-d H:i:s')]);
                    ...
                    $cek_all_status = TransaksiTransferDetail::on($this->getConnectionName())->where('id_nota', $tf->id_nota)->where('is_status', 0)->count();
                    if($cek_all_status < 1) {
                        TransaksiTransfer::on($this->getConnectionName())->where('id', $tf->id_nota)->update(['is_status' => 1, 'updated_at' => date('Y-m-d H:i:s'), 'updated_by' => Auth::id()]);
                    }
                }
