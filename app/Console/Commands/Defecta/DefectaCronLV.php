...
class DefectaCronLV extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'defectalv:cron';
    ...
    public function handle()
    {
        ...
        if(!empty($cek)) {
            ...
            if($last_id_obat_ex >= $last_id_obat) {
                # selesai : 1. hapus data di tb_bantu_transaksi_update_lv dan ulang dari 0
                DB::connection($this->getConnectionName())->table('tb_bantu_transaksi_update_lv')->truncate();
                $skip = 1;
            } else {
