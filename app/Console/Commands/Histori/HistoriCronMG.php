...
class HistoriCronMG extends Command
{
    ...
    public function handle()
    {
        ...
        $cek = DB::connection($this->getConnectionName())->table('tb_bantu_update_mg')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
            ...
            if($last_id_obat_ex >= $last_id_obat) {
                ...
                DB::connection($this->getConnectionName())->table('tb_bantu_update_mg')->truncate();
                $skip = 1;
            } else {
        ...
        if($skip != 1) {
            DB::connection($this->getConnectionName())->table('tb_bantu_update_mg')
