...
class HistoriCronMG extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historimg:cron';
    ...
    public function handle()
    {
        ...
        $cek = DB::table('tb_bantu_update_mg')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
            $last_id_obat_ex = $cek->last_id_obat_after;
            if($last_id_obat_ex >= $last_id_obat) {
                # selesai : 1. hapus data di tb_bantu_update_mg dan ulang dari 0
                DB::table('tb_bantu_update_mg')->truncate();
                $skip = 1;
            } else {
