...
class ReloadHistoriOld extends Command
{
    ...
    public function handle()
    {
        ...
        $id_apotek = 1;
        $skip = 0;
        $cek = DB::connection($this->getConnectionName())->table('tb_bantu_update_old')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
        ...
        if($skip != 1) {
            ...
            foreach ($data as $key => $obj) {
                $det_historis = DB::connection($this->getConnectionName())->table('tb_histori_all_'.$inisial.'')->where('id_obat', $obj->id_obat)->whereIn('id_jenis_transaksi', [1, 2, 3, 4, 14, 15, 16, 17])->get();
                foreach ($det_historis as $key => $val) {
                    if($val->id_jenis_transaksi == 1) {
                        # penjualan
                        $det_  = TransaksiPenjualanDetail::on($this->getConnectionName())->find($val->id_transaksi);
                        $nota = $det_->nota;
                        $created_at = $nota->created_at;
                    } else if($val->id_jenis_transaksi == 2) {
                        # pembelian
                        $det_  = TransaksiPembelianDetail::on($this->getConnectionName())->find($val->id_transaksi);
                        $nota = $det_->nota;
                        $created_at = $nota->created_at;
                    } else if($val->id_jenis_transaksi == 3) {
                        # transfer masuk
                        $det_  = TransaksiTODetail::on($this->getConnectionName())->find($val->id_transaksi);
                        $nota = $det_->nota;
                        $created_at = $nota->created_at;
