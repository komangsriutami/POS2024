...
<div class="row">
    <div class="form-group col-md-2">
        ...
        {!! Form::text('no_transaksi', $jurnal_penyesuaian->no_transaksi, array('class' => 'form-control required', 'placeholder'=>'Masukan No. Transaksi')) !!}
    </div>
    <div class="form-group col-md-2">
        {!! Form::label('tgl_transaksi', 'Tgl. Transaksi') !!}
        {!! Form::text('tgl_transaksi', $jurnal_penyesuaian->tgl_transaksi, array('id' => 'tgl_transaksi','class' => 'form-control required', 'placeholder'=>'Masukan Tanggal Transaksi', 'autocomplete' => 'off')) !!}
    </div>
    ...
    */?>
    ...
    <div class="form-group col-md-4">
        {!! Form::label('tag', 'Tag') !!}
        {!! Form::text('tag', $jurnal_penyesuaian->tag, array('id' => 'tag','class' => 'form-control', 'placeholder'=>'Gunakan koma (,)', 'autocomplete' => 'false')) !!}
    </div>

    <?php /*
    <div class="form-group col-md-3">
        {!! Form::label('kode_referensi', 'Kode Referensi / Kontak') !!}
        {!! Form::text('kode_referensi', $jurnal_penyesuaian->kode_referensi, array('id' => 'kode_referensi','class' => 'form-control', 'placeholder'=>'', 'autocomplete' => 'false')) !!}
    </div>
    */ ?>
</div>

<div class="row"><div class="col-sm-12"><label>Detail Jurnal</label></div></div>
<input type="hidden" id="count" value="{{$jurnal_penyesuaian->detailjurnal->count()}}">
...
<div id="div_detail">
    @if($jurnal_penyesuaian->detailjurnal->count())
        @foreach($jurnal_penyesuaian->detailjurnal as $key => $d)
            @include('jurnal_penyesuaian/_form_detail', ['count' => $key,'detailjurnal'=>$d,"kode_akun"=>$kode_akun])
        @endforeach
    @endif
</div>
...
<div class="row mt-4">
    <div class="col-sm-4">
        <div class="form-group">
            <label>Memo</label>
            <textarea class="form-control" name="memo">{{$jurnal_penyesuaian->memo}}</textarea>  
        </div>        
    </div> 
...
<input type="hidden" id="countfile" value="{{$jurnal_penyesuaian->filebuktijurnal->count()}}">
<div id="div_bukti">
    @if($jurnal_penyesuaian->filebuktijurnal->count())
        @foreach($jurnal_penyesuaian->filebuktijurnal as $key => $f)
            @include('jurnal_penyesuaian/_form_file', ['count' => $key,'filebukti'=>$f])
        @endforeach
    @endif
</div>
