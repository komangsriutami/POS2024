...
@section('content')
	...
	<div class="card card-info card-outline mb-12 border-left-primary">
	    <div class="card-body">
			...
			<a class="btn btn-success w-md m-b-5" href="{{url('pengumuman/create')}}"><i class="fa fa-plus"></i> Tambah Data</a>
	    </div>
	</div>
	...
	<div class="card card-info card-outline" id="main-box" style="">
        ...
        <div class="card-body">
			<table  id="tb_pengumuman" class="table table-bordered table-striped table-hover">
		    	<thead>
			        <tr>
			            <th width="3%">No.</th>
...
<script type="text/javascript">
	...
	var tb_pengumuman = $('#tb_pengumuman').dataTable( {
			processing: true,
	        serverSide: true,
	        stateSave: true,
	        ajax:{
			        url: '{{url("pengumuman/list_pengumuman")}}',
			        data:function(d){
				         }
			     },
	        columns: [
	            {data: 'no', name: 'no',width:"2%", class:'text-center'},
	            {data: 'id_role_penerima', name: 'id_role_penerima'},
	            {data: 'id_asal_pengumuman', name: 'id_asal_pengumuman'},
		...
		function(){
			$.ajax({
				...
				success:  function(data){
					if(data==1){
						swal("Deleted!", "Data pengumuman berhasil dihapus.", "success");
					}else{
						
						swal("Failed!", "Gagal menghapus data pengumuman.", "error");
					}
				},
				complete: function(data){
					tb_pengumuman.fnDraw(false);
				},
				error: function(data) {
	...
	function submit_valid(id){
		if($(".validated_form").valid()) {
			...
			$.ajax({
				...
				success:  function(data){
					if(data.status ==1){
						show_info("Data pengumuman berhasil disimpan!");
	...
	function edit_data(id){
      	$.ajax({
	        ...
	        beforeSend: function(data){
		        ...
		        $("#modal-xl .modal-title").html("Edit Data - Pengumuman");
		        $('#modal-xl').modal("show");
