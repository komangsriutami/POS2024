...
<div class="card card-info card-outline mb-12 border-left-primary">
    <div class="card-body">
        ...
        <a class="btn btn-success w-md m-b-5" href="{{url('input_aset/create')}}"><i class="fa fa-plus"></i> Tambah Data</a>
    </div>
</div>
...
<script type="text/javascript">
    ...
    var tb_aset = $('#tb_aset').dataTable({
        ...
        stateSave: true,
        ajax: {
            url: '{{url("input_aset/list_data_aset")}}',
            data:function(d){
    ...
    function detail_data(id) {
        $.ajax({
            type: "GET",
            url: '{{url("input_aset")}}/' + id + '/edit_detail',
            async: true,
            data: {
                _token: "{{csrf_token()}}",
                id: "id"
    ...
    function submit_valid(id) {
        if ($(".validated_form").valid()) {
            data = {};
            $("#form-edit").find("input[no_transaksi], select").each(function(index, node) {
                data[node.no_transaksi] = node.value;
            });
            $.ajax({
                type: "PUT",
                url: '{{url("input_aset/")}}/' + id,
                ...
                success: function(data) {
                    if (data.status == 1) {
                        show_info("Data master aset berhasil disimpan!");
